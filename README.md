...
- 全局说明
    ...
    - 网络异常
        - 网络切换
          需求：当网络环境从wifi转换到移动网络时，提示用户，避免浪费大量流量，更加人性化
          界面设计：
          1）阻断式操作，暂停用户端操作，弹出警告框，需用户手动确认是否继续程序操作
          2）非阻断式提醒，在用户界面显示提醒内容，一定时间后自动消失，不干预用户任何程序操作
        - 断网情况
          需求：当应用检测到目前为无网络数据时，为了保证用户继续使用，需提醒用户
          页面设计：
          1）整页提示
          2）警告框
          3）tips提示
